class HealthInterviewsController < ApplicationController
  ...
  before_action :set_guide_statuses, only: [:index, :update]
  before_action :patient_required, only: [:new]
  before_action :staff_required, only: [:show, :edit]
  # before_action :admin_required, only: [:show]

  def index
    @health_interviews = HealthInterview.with_guide_status.order(created_at: :asc)
  ...
  def new
    ...
    # unless @health_interviews.search_initial.already(current_patient) ||
    #        @health_interviews.search_calling.already(current_patient) ||
    #        @health_interviews.search_pending.already(current_patient)
    @health_interviews = current_patient.health_interviews
    if @health_interviews.present?
      unless @health_interviews.last.guide_status.done? || @health_interviews.last.guide_status.noshow?
  ...
  def update
    if @health_interview.update(health_interview_params)
      redirect_to health_interviews_path, notice: t('notice.update')
    else
      render :edit
    end
  end
  ...
  def set_guide_statuses
    @guide_statuses = GuideStatus.all
  end
